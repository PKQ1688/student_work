...
def prepare_example(example):
    audio_path = f"phonetic wav/{example['filename']}"
    speech_array, sr = librosa.load(audio_path, sr=16000)  # Assuming librosa is already importing
    example['speech'] = {'array': np.array(speech_array, dtype=np.float32), 'sampling_rate': sr}
    return example
